...
Extension SDK manifest needs to be placed in the root of the package directory (same as *.nuspec) and is optional. Manifest is primarily required to specify metadata used by the designer tools in Visual Studio and if you are not shipping controls then it is not required.
