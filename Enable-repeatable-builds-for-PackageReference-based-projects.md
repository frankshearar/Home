...
This will be an intrinsic feature for NuGet and hence the lock file should be generated by default for all NuGet restores. However, the feature would be behind a feature flag to start with. This will let us roll out the feature without impacting developers who do not expect to see an additional file. 
...
* The lock file will be called `nuget.packages.lock` 
...
* Simple format (similar to a yaml file):
